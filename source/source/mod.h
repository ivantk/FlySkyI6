...
__attribute__((section(".mod_unknownSensor"))) const uint8_t UNKNOWN_SENSOR[] = "Unknown";
...
const uint32_t __attribute__((section(".mod_extraMenu"))) menuList[] = {
		TEXT_TIMMER,
		(uint32_t)(&TimerConfig + 1),
		(uint32_t)(extraMenu + EXTRA_MENU_ALARM),
		(uint32_t)(&AlarmConfig + 1),
		(uint32_t)(auxSW + 6),
		(uint32_t)(&SwBConfig + 1),
		(uint32_t)(extraMenu + EXTRA_MENU_TXBAT),
		(uint32_t)(&BatteryType + 1),
		(uint32_t)&varioSensor,
		(uint32_t)(&varioSensorSelect + 1),
		(uint32_t)(extraMenu + EXTRA_MENU_ASL),
		(uint32_t)(&ASLConfig + 1),
		(uint32_t)TEXT_END_POINTS,
		(uint32_t)(&mixConfig + 1),
};
