...
const uint8_t *getSensorName(int sensor)
{
	...
	{
		if (sensor <= IBUS_MEAS_TYPE_FLIGHT_MODE)
			return SENSOR_00 + (sensor * 5);
		if (sensor == IBUS_MEAS_TYPE_PRES)
			return SENSOR_41;
		if (sensor < IBUS_MEAS_TYPE_ODO1)
...
void BatteryType()
{
	...
	while (1)
	{
		...
		if (key == KEY_SHORT_UP)
			batteryVolt++;
		else if (key == KEY_LONG_UP)
			batteryVolt += 50;
		else if (key == KEY_SHORT_DOWN)
			batteryVolt--;
		else if (key == KEY_LONG_DOWN)
			batteryVolt -= 50;
		else
			break;
	}
...
void CheckCustomAlarms()
{
	...
	{
		if ((uint32_t)configPtr->timerAlarm != 0 && timerValue > (uint32_t)configPtr->timerAlarm && timerValue < (uint32_t)configPtr->timerAlarm + TIMER_ALARM_DURATION)
		{
		...
		if (1)
		{ // ignore the lastTelemetryUpdate
#endif

			// Vario sound feedback
			sensorID = configPtr->varioSensorID;

			if (sensorID != IBUS_MEAS_TYPE_UNKNOWN && timer - varioPrevTime >= 1000 && !wasAlarm)
			{
...
void ASLConfig()
{
	...
	do
	{
		...
		for (uint8_t index = 0; index < 3; index++)
		{
			...
			if (index == 0)
				sprintfCall(buffer, (const char *)formatNumber, values[0]);
			if (index == 1)
				formatSensorValue(buffer, IBUS_MEAS_TYPE_TEM, (uint16_t)(values[1] - 400));
			if (index == 2)
				formatSensorValue(buffer, IBUS_MEAS_TYPE_ALT, IBUS_MEAS_TYPE_ALT - IBUS_MEAS_TYPE_GPS_LAT);
	...
	do
	{
		while (1)
		{
			...
			for (int i = 0; i < 3; i++)
			{
				...
				buffer[0] = 0;
				buffer[1] = 0;
				if (alarmItem.operator == OPERATOR_LT)
					buffer[0] = '<';
				if (alarmItem.operator == OPERATOR_GT)
					buffer[0] = '>';
			...
			else if (key == KEY_SHORT_UP || key == KEY_LONG_UP)
			{
				...
				if (column == 1)
				{
					if (alarms[row].operator != 0)
						alarms[row].operator = 0;
					else
						alarms[row].operator = 1;
				}
				if (column == 0)
				{
					alarms[row].value = 0;
...
void TimerConfig()
{
	...
	do
	{
		while (1)
		{
			...
			{
				data[navPos] += step;
				if (data[navPos] >= timerMaxValues[navPos])
					data[navPos] = timerMaxValues[navPos];
			}
			else if (key == KEY_SHORT_DOWN || key == KEY_LONG_DOWN)
			{
				if (data[navPos] >= step)
...
void varioSensorSelect()
{
	...
	do
	{
		...
		if (key == KEY_SHORT_UP || key == KEY_LONG_UP)
		{
			if (row == 0)
			{
				values[0] = nextSensorID(values[0]);
			}
			else if (row == 1)
			{
				if (values[1] < VARIO_MAX_GAIN)
					values[1]++;
			}
			else
			{
		...
		if (key == KEY_SHORT_DOWN || key == KEY_LONG_DOWN)
		{
			if (row == 0)
			{
				values[0] = prevSensorID(values[0]);
			}
			else if (row == 1)
			{
				if (values[1] > 1)
